...
| C7 fonts | *(opt-in — `FleetAppConfig.withBundledFonts`)* Every character printed under `lib/` is drawable by the app's own bundled fonts (format-4 cmap intersection); emoji and `RegExp(`/`// not-rendered` lines are exempt. Written so it cannot pass by finding nothing. |
