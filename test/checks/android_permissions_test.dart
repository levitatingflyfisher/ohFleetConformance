...
''');
  ...
  group('merged-manifest deepening (C4 v2)', () {
    ...
    test('the AGP-8 singular merged_manifest layout is also found', () {
      // Newer AGP writes build/app/intermediates/merged_manifest/release/
      // processReleaseMainManifest/AndroidManifest.xml (singular, no abi
      // dir) — a check blind to it would silently skip the whole v2
      // comparison on modern builds.
      final file = File(
        '${root.path}/build/app/intermediates/merged_manifest/release/'
        'processReleaseMainManifest/AndroidManifest.xml',
      );
      file.createSync(recursive: true);
      file.writeAsStringSync(manifestWith([notifications, internet]));
